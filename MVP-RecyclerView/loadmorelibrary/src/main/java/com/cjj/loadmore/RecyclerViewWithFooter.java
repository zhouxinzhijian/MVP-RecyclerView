...
public class RecyclerViewWithFooter extends RecyclerView {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    ...
                    if (layoutManager instanceof LinearLayoutManager) {
                        ...
                        if (lastVisiblePosition >0 && lastVisiblePosition>= recyclerView.getAdapter().getItemCount() - 1) {
    ...
    public void loadMoreIfAtTheEndOfTheList(){
        if(mOnScrollListener != null){
            mOnScrollListener.onScrollStateChanged(this, RecyclerView.SCROLL_STATE_IDLE);
        }
    }
     ...
     * 切换为滑动加载更多状态
     */
    public void setPullToLoad(CharSequence end) {
        if (getAdapter() != null) {
            mState = STATE_PULL_TO_LOAD;
            mFootItem.pullToLoadText = end;
            mIsGetDataForNet = false;
            mFootItem.onBindData(this, mState);
//            getAdapter().notifyItemChanged(getAdapter().getItemCount() - 1);
        }
    }
     ...
     * @param end 提示字符串
     */
    public void setEnd(CharSequence end, @DrawableRes int resId) {
        if (getAdapter() != null) {
            mState = STATE_END;
            mFootItem.endText = end;
            mFootItem.endIconRes = resId;
            mIsGetDataForNet = false;
            getAdapter().notifyItemChanged(getAdapter().getItemCount() - 1);
        }
    }
